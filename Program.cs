namespace Modbus.Containers
{
    ...
    class Program
    {
        ...
        static async Task<MessageResponse> PipeMessage(Message message, object userContext)
        {
            ...
            if (File.Exists(@"inputmessages.txt")) File.AppendAllText("inputmessages.txt",messageString+ Environment.NewLine);
        ...
        static async Task UpdateStartFromTwin(TwinCollection desiredProperties, ModuleClient ioTHubModuleClient)
        {
            ...
            if (desiredProperties.Contains(ModbusSlaves))
            {
                ...
                config = JsonConvert.DeserializeObject<ModuleConfig>(serializedStr);
                m_interval = JsonConvert.DeserializeObject<ModbusPushInterval>(serializedStr);
                m_version = JsonConvert.DeserializeObject<ModbusVersion>(serializedStr);
            }
            ...
            if (!string.IsNullOrEmpty(jsonStr))
            {
                ...
                localConfig = JsonConvert.DeserializeObject<ModuleConfig>(jsonStr);
                if (m_interval == null)
                {
                ...
                if (config != null)
                {
                    foreach (var slave in localConfig.SlaveConfigs)
                    {
                        Console.WriteLine("Merging Configuration for: " + slave.Key);
                        if (config.SlaveConfigs.ContainsKey(slave.Key))
                        {
                            Console.WriteLine("Assiging Operations for: " + slave.Key);
                            config.SlaveConfigs[slave.Key].Operations = slave.Value.Operations;
                        }
                        else
                        {
                            Console.WriteLine("Assiging Slave: " + slave.Key);
                            config.SlaveConfigs[slave.Key] = slave.Value;
                        }
                    }
                }
                else
        ...
        public static async Task<MethodResponse> ReceiveMessage(MethodRequest methodRequest, object userContext)
        {
            var data = Encoding.UTF8.GetString(methodRequest.Data);
            List<string> executionResult = null;
            MethodResponse methodResponse = null;

            if (methodRequest.DataAsJson != null)
            {
                var messageRecevied = new MessageProcessor();
                Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Mesagge recevied" + "\n");
                executionResult = await messageRecevied.WriteRegister(methodRequest.DataAsJson, userContext);
                Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "WriteRegister excetued" + "\n");
                var outResult = JsonConvert.SerializeObject(executionResult);//serialzing it into json format to send as json playload
                methodResponse = new MethodResponse(Encoding.UTF8.GetBytes(outResult), (int)HttpStatusCode.OK);
            }
            else
            {
                string result = "{\"result\":\"Executed Failed! Invalid Arguments\"}";
                methodResponse = new MethodResponse(Encoding.UTF8.GetBytes(result), (int)HttpStatusCode.InternalServerError);
            }
        ...
        {
            if (File.Exists(@"messagecount.txt"))
            {
                try
                {
                    StreamReader sr = new StreamReader(@"messagecount.txt");
                    string line = sr.ReadLine();
                    int count = Int32.Parse(line);
                    count++;
                    sr.Close();
                    StreamWriter sw = new StreamWriter(@"messagecount.txt");
                    sw.Write(count);
                    sw.Close();
                    return count;
        ...
        static async Task SentMessage(Message message, ModuleClient ioTHubModuleClient)
        {
            while (true)
            {
                try
                {
                    ...
                    Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Trying to send" + "\n");
                    await ioTHubModuleClient.SendEventAsync("modbusOutput", message, cancellationTokenSource.Token);
                    break;
                }
                catch (ArgumentException ex)
                {
                    ...
                    {
                        Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Message already presnt in DB" + "\n");
                        break;
                    }
                    else
                    {
                ...
                {
                    Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Message send to edgeHub Timedout");
            ...
            Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Message sent Successfully" + "\n");
            int measaagecount = MessageCount();
            Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "::" + "Message Count : " + measaagecount + "\n");
        }
        ...
        static async Task Start(object userContext)
        {
            ...
            while (m_run)
            {
                ...
                switch (m_version.Version)
                {
                    case "1":
                        ...
                        if (resultV1.Count > 0)
                        {
                            ...
                            string s = Encoding.UTF8.GetString(message.GetBytes());
                            if (File.Exists(@"messages.txt")) File.AppendAllText("messages.txt",s+ Environment.NewLine);
                        }
                ...
                if (message != null)
                {
                    await SentMessage(message, ioTHubModuleClient);
                }
